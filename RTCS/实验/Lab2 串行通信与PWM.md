...
**速率**由主机的预分频器决定。SPI1 挂在 APB2 上，本实验主频 64 MHz、预分频 128：

$$
f_{\text{SCK}} = \frac{64\ \text{MHz}}{128} = 500\ \text{kbit/s}
\qquad\Rightarrow\qquad
\text{一个字节} = \frac{8}{500\,000} = 16\ \mu\text{s}
$$
...
> **最大只到 90**（正文说"最亮"达不到）、**一轮 9 秒**（正文说 10 秒）、**初值 10 因为先自增而从未被输出过**。
...
| §2.2 从机地址 | `0b1101001 or 0x6E`，二者不等 | `0b1101001` = 0x69；0x6E = `0b1101110`。两者必居其一有误 |
| Figure 8a | 标 `R/W: Read` 但该位画的是 0，正文也说这是写 | 应为 `Write` |
| Table 1 | I2C 命令表与源码、与图 11a 三者互相矛盾 | 以 `i2c_arduino.ino` 源码为准，见 §4.4 |
| §2.3 正文 | "Arduino 能测的最大值是 1024" | 10 位 ADC 范围是 **0–1023**，1024 是量化级数 |
...
| PB6 | §2.3.1 当 SPI 片选、§3.2 当 TIM4_CH1 | **同一引脚不能兼任**，片选改用 PC7 |
